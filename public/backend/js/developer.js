...
$(document).ready(function () {
    $('body').on("click", ".modalLink", function (e) {
        ...
        let data_title = $(this).attr('data-original-title');
...
function calculateSalary() {
    ...
    if (basicSalary == 0) {
        ...
        let earningsType = document.getElementsByName('earningsValue[]');
        let earningsValue = document.getElementsByName('earningsValue[]');
        let tax = $("#tax").val();
        let total_earnings = 0;
        let total_deduction = 0;
        let deductionstype = document.getElementsByName('deductionstype[]');
        let deductionsValue = document.getElementsByName('deductionsValue[]');
        for (let i = 0; i < earningsValue.length; i++) {
            let inp = earningsValue[i];
            if (inp.value == '') {
                let inpvalue = 0;
            } else {
        ...
        for (let j = 0; j < deductionsValue.length; j++) {
            let inpd = deductionsValue[j];
            if (inpd.value == '') {
                let inpdvalue = 0;
            } else {
                let inpdvalue = inpd.value;
            }
            total_deduction += parseInt(inpdvalue);
        }
        let gross_salary = parseInt(basicSalary) + parseInt(total_earnings) - parseInt(total_deduction);
        let net_salary = parseInt(basicSalary) + parseInt(total_earnings) - parseInt(total_deduction) - parseInt(tax);
...
function addRow() {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            let tr = '<tr id="row' + count + '" class="' + arrayNumber + '">' + '<td class="border-top-0"><div class="input-effect">';
...
function getTotalByPrice(row = null) {
    ...
    if (row) {
        ...
        if (product_id > 0) {
            let total = Number($("#unit_price" + row).val()) * Number($("#quantity" + row).val());
...
function subAmount() {
    let tableProductLength = $("#productTable tbody tr").length;
    ...
    let dueAmount = totalSubAmount - totalPaid;
...
function paidAmount() {
    ...
    if (subTotal) {
        let dueAmount = Number($("#subTotal").val()) - Number($("#totalPaid").val());
...
function delete_receive_payments(id) {
    let r = confirm("Are You Sure To delete This Payment ?");
    if (r == true) {
        event.preventDefault();
        let url = $("#url").val();
        let receive_payment_id = id;
        $.ajax({
            type: "POST",
            data: {
...
$('form[id="twilio_form"]').validate({
    ...
    submitHandler: function (form) {
        ...
        url = $("#url").val();
        let twilio_account_sid = $("#twilio_account_sid").val();
